...
const TimeSeriesChartInner = memo(function TimeSeriesChartInner({
  ...
  const timeSeriesData = useMemo<ChartPoint[]>(
    () => insertGapBreaks(dedupedData),
    [dedupedData],
  );
  ...
  const chartEvents = useMemo<
    Record<string, (evt: unknown) => void> | undefined
  >(() => {
    ...
    return {
      datazoom: (evt: unknown) => {
        ...
        const source =
          Array.isArray(payload.batch) && payload.batch.length > 0
            ? payload.batch[0]
            : payload;
