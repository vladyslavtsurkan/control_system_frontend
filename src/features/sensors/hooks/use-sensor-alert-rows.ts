...
import type { Alert, LiveAlert } from "@/features/alerts";
...
export function useSensorAlertRows({
  sensorId,
  alerts,
  liveAlerts,
}: UseSensorAlertRowsParams) {
  ...
  const latestLiveByRuleId = useMemo(() => {
    ...
    for (const alert of liveAlertsForSensor) {
      ...
      if (
        !existing ||
        Date.parse(alert.updated_at) > Date.parse(existing.updated_at)
      ) {
  ...
  return useMemo<SensorAlertRow[]>(() => {
    ...
    const restRows: SensorAlertRow[] = restItems.map((alert) => {
      ...
      const liveMatch =
        !alert.resolved_at && normalizedRuleId
      ...
      return {
        ...
        updatedAt:
          liveMatch?.updated_at ?? alert.updated_at ?? alert.created_at,
        resolvedAt:
    ...
    const liveOnlyRows: SensorAlertRow[] = liveAlertsForSensor
      .filter(
        (alert) =>
          alert.status === "active" && !restActiveRuleIds.has(alert.rule_id),
      )
      .map((alert) => ({
        key: `live-${alert.key}`,
        source: "live",
