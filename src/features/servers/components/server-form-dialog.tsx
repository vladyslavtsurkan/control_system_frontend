...
import { useCreateServerMutation, useUpdateServerMutation } from "@/store/api-slice";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogClose } from "@/components/ui/dialog";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import type { OpcServer, AuthMethod, SecurityPolicy, CreateOpcServerRequest } from "@/types/models";
...
const emptyForm: ServerFormState = {
  name: "", description: "", url: "", security_policy: "None",
  authentication_method: "anonymous", username: "", password: "",
};
...
export function ServerFormDialog({ open, onOpenChange, editTarget }: ServerFormDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editTarget) {
        await updateServer({
          id: editTarget.id, name: form.name || undefined,
          description: form.description || null, url: form.url || undefined,
        ...
        const payload: CreateOpcServerRequest = {
          name: form.name, description: form.description || null, url: form.url,
          security_policy: form.security_policy,
          authentication_method: form.authentication_method,
          username: form.username || null, password: form.password || null,
        };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{editTarget ? "Edit OPC UA Server" : "Add OPC UA Server"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="server-name">Name</Label>
            <Input id="server-name" required value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Production Line A" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="server-desc">Description</Label>
            <Input id="server-desc" value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="Optional description" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="endpoint-url">Endpoint URL</Label>
            <Input id="endpoint-url" required value={form.url} onChange={(e) => setForm((f) => ({ ...f, url: e.target.value }))} placeholder="opc.tcp://192.168.1.100:4840" />
          </div>
          ...
          {showCredentials && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input id="opc-username" autoComplete="off" value={form.username} onChange={(e) => setForm((f) => ({ ...f, username: e.target.value }))} placeholder="opcuser" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="opc-password">Password{editTarget ? " (leave blank to keep)" : ""}</Label>
                <Input id="opc-password" type="password" autoComplete="new-password" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} placeholder="••••••••" />
              </div>
            </div>
          )}
          <DialogFooter>
            <DialogClose render={<Button type="button" variant="outline" />}>Cancel</DialogClose>
            <Button type="submit" disabled={creating || updating}>{editTarget ? "Save Changes" : "Create Server"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
