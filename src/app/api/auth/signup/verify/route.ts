...
export async function POST(req: NextRequest) {
  ...
  const backendRes = await fetch(
    `${BACKEND_API_URL}/api/v1/auth/signup/verify`,
    {
      method: "POST",
  ...
  if (!backendRes.ok) {
    const error = await backendRes
      .json()
      .catch(() => ({ detail: "Signup verification failed" }));
    return NextResponse.json(error, { status: backendRes.status });
  }

  const { access_token, refresh_token } =
    (await backendRes.json()) as SignUpVerifyResponse;
  ...
  if (!meRes.ok) {
    const error = await meRes
      .json()
      .catch(() => ({ detail: "Could not load user profile" }));
